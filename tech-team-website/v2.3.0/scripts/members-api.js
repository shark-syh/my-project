...
hexo.extend.filter.register('server_middleware', function (app) {
  ...
  function validateAvatar(avatar) {
    ...
    var a = avatar.trim();
    if (/["<>]/.test(a)) return 'avatar 路径包含非法字符';
    if (/^(https?:)?\/\//.test(a)) return 'avatar 不允许使用外部 URL';
    return null;
  }
  ...
  app.use('/api/members', function (req, res, next) {
    ...
    if (req.method === 'POST') {
      ...
      var body = '';
      var BODY_LIMIT = 2048;
      req.on('data', function (chunk) { body += chunk; if (body.length > BODY_LIMIT) { reply(res, 413, { error: '请求体过大' }); req.destroy(); return; } });
      req.on('end', function () {
        ...
        // 校验 avatar
        var avatarErr = validateAvatar(data.avatar || '');
        if (avatarErr) { reply(res, 400, { error: avatarErr }); return; }
        ...
        audit.log(baseDir, 'MEMBER_ADD', auth.getUsername(auth.getToken(req)), '添加成员「' + newMember.name + '」(' + newSlug + ')', req);
    ...
    if (req.method === 'PUT') {
      ...
      var bodyPut = '';
      var BODY_LIMIT_PUT = 2048;
      req.on('data', function (chunk) { bodyPut += chunk; if (bodyPut.length > BODY_LIMIT_PUT) { reply(res, 413, { error: '请求体过大' }); req.destroy(); return; } });
      req.on('end', function () {
        if (bodyPut.length > BODY_LIMIT_PUT) return;
        var dataPut;
        ...
        // 校验 avatar
        var avatarErrPut = validateAvatar(dataPut.avatar || '');
        if (avatarErrPut) { reply(res, 400, { error: avatarErrPut }); return; }
        ...
        audit.log(baseDir, 'MEMBER_EDIT', auth.getUsername(auth.getToken(req)), '编辑成员 ' + targetSlugPut, req);
    ...
    if (req.method === 'DELETE') {
      ...
      audit.log(baseDir, 'MEMBER_DEL', auth.getUsername(auth.getToken(req)), '删除成员「' + removed.name + '」(' + targetSlug + ')', req);
