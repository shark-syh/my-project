(function () {
  ...
  var memberSection = document.querySelector('#team') || document.querySelector('.member-grid');
  if (!memberSection) return;

  var addBtn = null;
  var editorOverlay = null;
  var loggedIn = false;
  var editingSlug = null;   // null = 添加模式，字符串 = 编辑对应 slug 的成员
  var memberCache = [];     // 缓存的成员列表
  ...
  function createEditor() {
    ...
    editorOverlay.className = 'member-editor-overlay';

    var panel = document.createElement('div');
    panel.className = 'member-editor-panel';
  ...
  function bindCardClicks() {
    ...
    cards.forEach(function (card) {
      ...
      if (card.dataset.editorBound) return;
      card.dataset.editorBound = '1';
