(function () {
  ...
  function showToast(msg, type) {
    ...
    toastEl.className = 'welcome-toast ' + (type === 'error' ? 'error' : 'success');
  ...
  function createDialog() {
    ...
    dialog.className = 'admin-login-overlay overlay';

    var panel = document.createElement('div');
    panel.className = 'admin-login-panel overlay-panel';
    ...
    fetch('/api/auth', {
      ...
      .then(function (data) {
        if (data.ok) {
          ...
          hideDialog();
          if (name) showToast('欢迎 <span class="toast-username">' + escapeHTML(name) + '</span>', 'success');
        } else {
          if (!msgEl) msgEl = dialog.querySelector('.admin-login-msg');
