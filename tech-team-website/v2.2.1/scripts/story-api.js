...
hexo.extend.filter.register('server_middleware', function (app) {
  ...
  app.use('/api/auth', function (req, res, next) {
    ...
    var body = '';
    var AUTH_BODY_LIMIT = 512; req.on("data", function (chunk) { body += chunk; if (body.length > AUTH_BODY_LIMIT) { reply(res, 413, { error: "请求体过大" }); req.destroy(); return; } });
    req.on('end', function () {
      var data;
      try { data = JSON.parse(body); } catch (e) {
        reply(res, 400, { ok: false, error: 'JSON 解析失败' });
        return;
      }
  ...
  app.use('/api/story', function (req, res, next) {
    ...
    if (req.method === 'POST') {
      ...
      var body = ""; var STORY_BODY_LIMIT = 16384; req.on("data", function (chunk) { body += chunk; if (body.length > STORY_BODY_LIMIT) { reply(res, 413, { error: "请求体过大" }); req.destroy(); return; } });
      req.on('end', function () {
        var data;
        try { data = JSON.parse(body); } catch (e) {
